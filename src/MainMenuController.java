...
public class MainMenuController extends Thread {
    ...
    @FXML
    private TextField EditGiantbombID;
    @FXML
    private Label ImageURL;
    @FXML
    private Button LaunchGames;
    @FXML
    private Label GiantbombID;
    @FXML
    private Label size;
    @FXML
    private Slider page;
    ...
    public void setData(ArrayList<Game> games) {
        for (int i = 0; i < games.size(); i++) {
            try {
                if (games.get(i).getImage().equals("")) {
                    games.remove(i);
                    i--;
                }
    ...
    void loadimages(Image cover, int i) {
        try {
            ...
            //Exception doesn't deliver problems
        }
    }
    ...
    private void ShowDetails(int button) {
        if (!DetailEditMode) {
            ...
            if ((Target.getText() + SaveLocation.getText() + EditGameTitle.getText() + EditImageURL.getText() + EditDeck.getText()).equals("")) {
                ...
                EditGiantbombID.setText(SelectedGame.getID());
            } else {
                ...
                SelectedGame.setID(EditGiantbombID.getText());

                File old = new File("SaveData\\" + SelectedGame.getID() + ".png");
                if (old.exists())
                    old.delete();
            ...
            EditGiantbombID.setVisible(false);
            GiantbombID.setVisible(false);
        } else {
            ...
            EditGiantbombID.setVisible(true);
            GiantbombID.setVisible(true);
    ...
    public void uploadLibrary(String string) {
        Database_controller Db = new Database_controller();
        Db.clearComparison(string);
        username = string;

        Thread th = new Thread(this, "Database_controller");
        th.start();
        //Db.addLibrary(game.getGameTitle(), string);

    }

    public void run() {
        for (Game game : games) {
            new Database_controller(game.getGameTitle().replace("'", ""), username);
        }
    }
    ...
    public void handleRandom() {
        if (games.isEmpty()) {
            ...
            alert.setHeaderText(null);
            alert.setContentText("You have no games to randomize between");
            alert.showAndWait();
        } else {
            clearDisplay();
    ...
    public ObservableList<Compare> compare(String string) {
        Database_controller Db = new Database_controller();

        return Db.checkLibrary(string);
    }
    ...
    public void handleLaunchGame() {
        popUp(LaunchGames);
        ProcessBuilder builder = new ProcessBuilder("" + SelectedGame.getExe());
        builder.directory(SelectedGame.getExe().getParentFile());
        try {
            builder.start();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void handleSearch() {
        if (games.isEmpty()) {
            ...
            alert.setTitle("ERROR");
            alert.setHeaderText(null);
            alert.setContentText("You have no games to search");
            alert.showAndWait();
        } else {
